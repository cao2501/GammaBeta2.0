...
async function checkRingOwnership(kernel: Kernel, guildId: string, userId: string, ringItemId: string): Promise<boolean> {
...
export default class MarryCommand implements ICommand {
  data = new SlashCommandBuilder()
    ...
    .addSubcommand(s => s.setName('proposal').setDescription('💍 Gửi lời cầu hôn tới thành viên khác')
      .addUserOption(o => o.setName('user').setDescription('Đối tượng cầu hôn').setRequired(true))
      .addStringOption(o => o.setName('ring').setDescription('ID/Tên Nhẫn đính hôn (Cần mua sẵn ở cửa hàng)').setRequired(true))
    )
    .addSubcommand(s => s.setName('profile').setDescription('📋 Xem thông tin cuộc hôn nhân của bạn')
    ...
    .addSubcommand(s => s.setName('caption').setDescription('✨ Đổi lời chú thích (caption)')
      .addStringOption(o => o.setName('text').setDescription('Nội dung caption').setRequired(true))
    )
    .addSubcommand(s => s.setName('thumbnail').setDescription('🖼️ Cài đặt ảnh nhỏ cho embed profile')
      .addStringOption(o => o.setName('url').setDescription('URL hình ảnh').setRequired(true))
    )
    .addSubcommand(s => s.setName('image').setDescription('🖼️ Cài đặt ảnh lớn cho embed profile')
      .addStringOption(o => o.setName('url').setDescription('URL hình ảnh').setRequired(true))
    )
    .addSubcommand(s => s.setName('color').setDescription('🎨 Thay đổi màu viền embed')
  ...
  async execute(interaction: any, kernel: Kernel): Promise<void> {
      ...
      collector.on('collect', async (i: any) => {
        if (i.customId === 'marry:accept') {
          ...
          if (pMarried || tMarried) {
            return void i.reply({ content: '❌ Một trong hai bạn đã kết hôn trước khi lời đề nghị này được chấp thuận!', ephemeral: true });
          }

          const stillHasRing = await checkRingOwnership(kernel, guildId, userId, ringItem.id);
          if (!stillHasRing) {
            return void i.reply({ content: '❌ Bạn không còn sở hữu nhẫn đính hôn này nữa!', ephemeral: true });
          }
    ...
    // ─── 2. PROFILE SUBCOMMAND ───────────────────────────────────────────
      ...
      if (!marriage) {
        return void interaction.editReply(
          targetUser.id === userId
            ? '❌ Bạn chưa kết hôn! Dùng nhẫn cưới và lệnh `/marry proposal` để kết hôn.'
            : `❌ **${targetUser.username}** hiện chưa kết hôn với ai.`
        );
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: '📅 Kỷ Niệm', value: `${marriage.marriedAt.toLocaleDateString('vi-VN')} (${daysDiff} ngày)`, inline: true },
    ...
    // ─── 3. DIVORCE SUBCOMMAND ───────────────────────────────────────────
      ...
      if (!marriage) {
        return void interaction.editReply('❌ Bạn hiện chưa kết hôn với ai.');
      }
      ...
      await interaction.editReply(`💔 **Ly Hôn:** <@${userId}> và <@${partnerId}> đã ly hôn. Chúc cả hai tìm được hạnh phúc mới.`);
    ...
    // ─── 4. LUV SUBCOMMAND ───────────────────────────────────────────────
      ...
      if (lastLuv && now.getTime() - lastLuv.getTime() < 24 * 3600 * 1000) {
        ...
        return void interaction.editReply(`⏳ Bạn đã farm điểm hôm nay rồi! Vui lòng quay lại sau **${remainingHours} giờ**.`);
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: '🔥 Chuỗi Liên Tục (Streak)', value: `${newStreak} ngày ${streakIncremented ? '📈' : '⚠️ Reset do quên'}`, inline: true },
    ...
    // ─── 5. CUSTOMIZATION SUBCOMMANDS ────────────────────────────────────
      ...
      if (!/^[0-9a-f]{6}$/.test(colorArg)) {
        return void interaction.editReply('❌ Mã màu không hợp lệ! Vui lòng nhập định dạng hex 6 ký tự (Ví dụ: `ff7bb5` hoặc `0xff7bb5`).');
      }
