  ...
  async execute(interaction: ChatInputCommandInteraction, kernel: Kernel): Promise<void> {
    ...
    if (sub === 'setup') {
      ...
      await interaction.reply({
        content: `✅ Xác minh **${type}** đã cấu hình!\nRole: ${role}\nDùng \`/verify panel\` để gửi panel.`,
        ephemeral: true,
      });
      ...
      if (!enabled || !config.type) {
        return void interaction.reply({ content: '❌ Chưa cấu hình verify. Dùng `/verify setup`.', ephemeral: true });
      }

      const channelId = config.channelId ?? interaction.channelId;
      const ch = kernel.client.channels.cache.get(channelId);
      if (!ch?.isTextBased()) return void interaction.reply({ content: '❌ Kênh verify không hợp lệ.', ephemeral: true });

      const embed = new EmbedBuilder()
        .setTitle('✅ Xác Minh Thành Viên')
        .setColor(0x57f287)
        .setDescription(
          config.type === 'BUTTON' ? '👋 Nhấn nút bên dưới để xác minh và truy cập server.' :
      ...
      const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder()
          .setCustomId(`verify:start:${config.type}`)
          .setLabel('✅ Xác Minh Ngay')
          .setStyle(ButtonStyle.Success),
      );

      await (ch as any).send({ embeds: [embed], components: [row] });
      await interaction.reply({ content: `✅ Panel verify đã gửi vào <#${channelId}>.`, ephemeral: true });
      ...
      await interaction.reply({
        content: `✅ Auto-kick ${enabled ? `bật — kick sau **${hours}h** chưa verify` : 'đã tắt'}.`,
        ephemeral: true,
      });
